...
import { GraduationCap, Zap, Users, ShieldCheck, Mail, Lock, User as UserIcon, Loader2, ArrowRight } from 'lucide-react';
...
export default function Landing() {
  ...
  const [useEmailAuth, setUseEmailAuth] = useState(true); // Default to true since popup is blocked in sandbox iframe
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-blocked') {
        setErrorMsg("Google Sign-In popup was blocked by your browser. Please use the Student Email form below, or open this app in a new tab.");
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        setErrorMsg("No account found with this email. Click 'Create Student Account' below.");
          ...
          {useEmailAuth ? (
            ...
            <div className="space-y-4">
              <button
                ...
                disabled={loading}
                className="w-full py-4 border border-slate-250/60 text-slate-800 bg-white rounded-2xl font-bold text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-3 active:scale-95"
              >
                {loading ? (
                  <Loader2 size={18} className="animate-spin" />
                ) : (
